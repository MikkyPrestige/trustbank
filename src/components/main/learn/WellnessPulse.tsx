...
export default function WellnessPulse({ settings }: WellnessPulseProps) {
    ...
    // Score is calculated out of 30 possible points (3 questions * 10 max)
            ...
            {!showResult ? (
                ...
                <div className={styles.resultBox}>
                    <div className={styles.scoreCircle} style={{ borderColor: feedback.color }}>
                        <span className={styles.scoreNumber} style={{ color: feedback.color }}>{totalScore}</span>
                        <span className={styles.scoreLabel}>/ 100</span>
                    </div>

                    <h2 style={{ color: feedback.color }}>{feedback.label}</h2>
                    <p>{feedback.text}</p>

                    <button className={styles.resetBtn} onClick={() => setShowResult(false)}>
                        {settings.learn_pulse_reset_btn} <ArrowRight size={16} />
                    </button>
                </div>
            )}
        </div>
